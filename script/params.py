...
s3 = sqrt(3)*(b*r/2)*eta
print("s1: %d s2: %d s3: %d" % (s1, s2, log2(s3)))

sig1 = 2*sqrt(3)*eta
sig2 = 2*sqrt(3)*sqrt(kp)*eta
sig3 = 2*sqrt(3)*(b*r/2)*eta
print("sig1: %d sig2: %d sig3: %d" % (sig1, sig2, log2(sig3)))
...
beta_open += ((m+1)*sig1 + sqrt(m+2)*sig2)**2 * mu*d
beta_open += (b+1)**2 * ((m+1)*s1 + sqrt(m+2)*s2)**2 * ell*d
beta_open = beta_open**0.5

beta_eval = ((m+1)*(b+1)*(r/2)*sig1 + sqrt(m+2)*sig3)**2 * nu*d
beta_eval += ((m+1)*(b+1)*(r/2)*sig1 + sqrt(m+2)*sig3)**2 * mu*d
beta_eval += (b+1)**2 * ((m+1)*(b+1)*(r/2)*s1 + sqrt(m+2)*s2)**2 * ell*d
...
commit_size = log2(q) * (m + kp + 2) * mu * d

open_size = kp * d * mu * log2(5*(m+1)*sig1 + 5 * sqrt(m+2)*sig2)
open_size += kp * d * nu * log2(5*(m+1)*sig1 + 5 * sqrt(m+2)*sig2)
open_size += kp * d * ell * log2(5*b*(m+1)*s1 + 5*b*sqrt(m+2)*s2)

eval_size = d * mu * log2(5*(m+1)*b*(r/2)*sig1 + 5 * sqrt(m+2)*sig3)
eval_size += d * nu * log2(5*(m+1)*b*(r/2)*sig1 + 5 * sqrt(m+2)*sig3)
eval_size += d * ell * log2(5*(m+1)*(b**2)*(r/2)*s1 + 5*b*sqrt(m+2)*s3)
